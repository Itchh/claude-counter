...
// Once a bar is segmented by model, colour means model — so the track behind
// it goes neutral rather than tinting to the person's identity colour, which
// would otherwise read as another model band.
const SEGMENTED_TRACK_COLOR = 'rgba(42, 42, 74, 0.28)'
...
export function StandardLeaderboard(): React.ReactElement {
  ...
  return (
    ...
    >
      ...
      >
                ...
                >
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    ...
                    <div
                      className="bar-track"
                      style={{
                        background: `repeating-linear-gradient(to right, ${GRIDLINE_COLOR} 0 1px, transparent 1px ${axis.stepPercent}%), ${
                          segments.length > 0
                            ? SEGMENTED_TRACK_COLOR
                            : barTrackColor(entry.rank, entry.color)
                        }`,
                      }}
                    >
                      <motion.div
        ...
        {/* TOKEN AXIS — mile markers aligned to the bar column */}
        {data && data.leaderboard.length > 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            ...
            <span className="online-dot" style={{ background: 'transparent', boxShadow: 'none' }} />
            <div style={{ flex: 1, position: 'relative', height: '20px' }}>
              {axis.ticks.map((tick) => (
                ...
                >
                  <span style={{ width: '1px', height: '4px', background: '#2a2a4a' }} />
                  <span
                    style={{
                      color: '#3a3a5a',
                  ...
                  >
                    {tick.value === 0 ? '0' : fmtTokensShort(tick.value)}
                  </span>
                </div>
              ))}
            </div>
            <span style={{ flex: '0 0 clamp(100px, 12vw, 160px)' }} />
          </motion.div>
        )}

        {/* MODEL LEGEND — colour key for the candy-bar segments */}
        {legend.length > 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
