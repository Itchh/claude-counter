...
export interface TrackSurface {
  ...
  /**
   * A texture page for a surface the source model shipped bare.
   *
   * Untextured geometry renders as one flat sheet of `color`, which at PS2
   * fidelity reads as a missing asset — the grey mountains, the grey
   * barriers. These pages are generated by scripts/makeSurfaceTextures.mjs
   * and projected in world space (the geometry has no UVs to read), so
   * `textureScale` says how many world units one repeat of the page covers.
   * Ignored when the source material carries its own map: the model's
   * authored pages always win.
   */
  readonly textureUrl?: string
  /** World units per tile of `textureUrl`. */
